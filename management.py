...
class ManagementPage(webapp2.RequestHandler):
    def get(self):
        ...
        for stream in streams:
            self.response.write('<tr><td><a href="%s">%s</a></td><td>%s</td><td>%s</td><td><input type="checkbox" name="status", value="%s"></td></tr>' % (stream.url,stream.name,stream.lastnewdate,stream.numberofpictures,stream.name))
